...
class bottrap(discord.ui.View) :
	...
	async def disable_buttons(self, interaction: discord.Interaction) :
		...
		try :
			...
			logging.debug(f"Bot trap: could not disable buttons: {e}")
	...
	async def trigger(self, interaction: discord.Interaction) :
		...
		if not AccessControl().is_premium(interaction.guild.id):
			...
			return
		logging.info(f"Bot trap triggered by {interaction.user} ({interaction.user.id}) in {interaction.guild.name} ({interaction.guild.id})")
		...
		if modchannel is None:
			logging.warning(f"Bot trap: no mod channel resolved for {interaction.guild.name} ({interaction.guild.id})")
			return
		...
		try :
			...
			logging.warning(f"Bot trap: missing permission to ban {interaction.user} ({interaction.user.id}) in {interaction.guild.name}")