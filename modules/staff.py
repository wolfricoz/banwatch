...
class staff(commands.GroupCog, name="staff"):
    ...
    async def userinfo(self, interaction: discord.Interaction, user: discord.User):
        ...
        guild_data = {
            "Common Servers": "\n".join([f"{guild.name}({guild.id})" for guild in self.bot.guilds if user in guild.members])[:1000],
            "Bans"          : len(ban_info),
        ...
        for key, value in guild_data.items():
            embed.add_field(name=key, value=value, inline=False)
        embed.set_footer(text=f"This data may only be used for investigations, and may never be used for non-banwatch related actions.")
        await send_message(interaction.channel, embed=embed)
